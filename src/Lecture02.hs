...
map _ Nil = Nil
map f xs  = foldr (\x y -> (f x) :. y) Nil xs
...
filter _ Nil = Nil
filter f xs  = foldr (\x y -> if f(x) then x :. y else y) Nil xs
...
intercalate :: [a] -> [[a]] -> [a]
intercalate _ [] = []
intercalate sep (x : xs) = P.foldl (\x y -> x ++ sep ++ y) x xs
